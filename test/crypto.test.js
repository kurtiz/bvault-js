...
import { base64ToBuffer, bufferToBase64, bufferToString, stringToBuffer, } from '../src/lib/converters.js';
...
async function withRealCrypto(fn) {
...
describe('Encryption Library', () => {
    describe('Unit Tests (Mocked)', () => {
        ...
        it('should throw DecryptionError on failure', async () => {
            ...
            await expect(decrypt('encrypted', 'pass', 'iv', 'salt')).rejects.toThrow(DecryptionError);
        });
    });
    describe('Integration Tests (Real Crypto)', () => {
        ...
        it('should decrypt data successfully', async () => {
            ...
            const result = await withRealCrypto(async () => {
                ...
                return await decrypt(encrypted.encryptedData, 'pass123', encrypted.iv, encrypted.salt);
            });
        ...
        it('should fail with wrong password', async () => {
            await withRealCrypto(async () => {
                ...
                await expect(decrypt(encrypted.encryptedData, 'wrong-pass', encrypted.iv, encrypted.salt)).rejects.toThrow(DecryptionError);
            });
        });
        it('should fail with tampered IV', async () => {
            await withRealCrypto(async () => {
                ...
                await expect(decrypt(encrypted.encryptedData, 'pass', tamperedIV, encrypted.salt)).rejects.toThrow(DecryptionError);
            });
        });
    });
